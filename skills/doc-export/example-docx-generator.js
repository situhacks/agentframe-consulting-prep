...
const out = CP + "\\workspace\\applications\\EXAMPLE-consulting-analyst\\stage-2-tailor\\";
Packer.toBuffer(cv).then(b => { fs.writeFileSync(out + "cv.docx", b); console.log("wrote cv.docx"); });
Packer.toBuffer(cl).then(b => { fs.writeFileSync(out + "cover-letter.docx", b); console.log("wrote cover-letter.docx"); });
